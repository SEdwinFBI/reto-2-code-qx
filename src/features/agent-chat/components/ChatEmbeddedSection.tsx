...
export const ChatEmbeddedSection: React.FC<ChatEmbeddedSectionProps> = ({
  ...
  return (
    <section ref={sectionRef} className="py-4 sm:py-5 bg-slate-50/60 border-t border-slate-200/60">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Section Header */}
        <div className="text-center mb-2.5">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold text-gold-800 tracking-wider uppercase mb-1">
            <Sparkles className="w-3.5 h-3.5" />
            ASISTENCIA INMEDIATA
          </span>
          <h3 className="font-display text-xl sm:text-2xl font-bold text-navy-900 tracking-tight mb-1">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200/90 overflow-hidden flex flex-col">
          {/* Header */}
          <div className="bg-navy-900 text-white px-4 sm:px-5 py-2.5 flex items-center justify-between shadow-xs shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="relative">
                <div className="w-8 h-8 rounded-full bg-navy-950 border border-gold-500/50 flex items-center justify-center shadow-inner">
                  <Shield className="w-4 h-4 text-gold-100" />
                </div>
          ...
          {/* Messages */}
          <div className="h-[340px] sm:h-[260px] overflow-y-auto overscroll-contain px-4 sm:px-5 py-2.5 bg-[#fbfcfe] space-y-2">
            {messages.map((msg) => (
