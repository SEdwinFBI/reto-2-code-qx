...
export function useSolicitud() {
  ...
  /**
   * No existe hoy un servicio real de identidad (RENAP/Tránsito): se simula la
   * consulta contra POST /api/consulta-persona, que a su vez busca en el banco de
   * perfiles de prueba (src/data/perfiles-prueba.json). Si hay coincidencia, se
   * prerellena todo el formulario (excepto los archivos); si no, se cae a llenado
   * manual. Cuando exista un servicio real, sustituir solo el endpoint consumido.
   */
